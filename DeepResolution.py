...
def MCR(components,X,RT,y,result_path,default_method): 
    ...
    for overpeak in range(len(peaks)):
        ...
        if len(peak) <= 3:
            ...
            if default_method == 'ITTFA':
                methodi = default_method
                if len(peak) == 1:
            ...
            if default_method =='MCRALS':
                ...
                if len(peak) ==1:    
                    ...
                    ch = np.abs(np.sum(x,1)) 
                    ind_s1 = np.argmax(ch[int(peak0[0]-peak0[0]+range_point-1):int(peak0[1]-peak0[0]+range_point)])
                    ind_s2 = np.argmax(ch[int(peak1[0]-peak0[0]+range_point):int(peak1[1]-peak0[0]+range_point+1)])
                    ind_s = [int(ind_s1+range_point),int(ind_s2+peak1[0]-peak0[0]+range_point)]                   
            ...
            print(methodi,', R2=',R2,',r2=',r2)
            endtime = datetime.datetime.now()  
            print (methodi,': The time of ', str(len(peak)), '-component:',(endtime - starttime),".seconds")    
...
if __name__ == '__main__':
    ...
    #default_method: ITTFA or MCRALS
    channle = 3
    default_method = 'ITTFA'
    ...
    component_file = csv.reader(open(work_path+'/data/component.csv', encoding='utf-8'))#data
...
#==============================================================================
    MCR(components,Xdata,RT,ypred,result_path,default_method)
